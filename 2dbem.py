...
def coincident_linear_galerkin(x, y, a, nu):
    """ After Galerkin integration with linear shape function 
        From showing_ben.ipnyb Sage notebook and maxima
        Do I also need the constant terms? What shape function are 
        they integrated over?"""
    f_galerkin = np.zeros((7, x.size))
    f_galerkin[0, :] = 1 / 4 * a ** 4 / (np.pi - np.pi * nu)
    f_galerkin[1, :] = 1 / 6 * a ** 3 / (nu - 1)
    f_galerkin[2, :] = 0
    f_galerkin[3, :] = 0
    f_galerkin[4, :] = -(2 * a** 2 * np.log(4 * a ** 2) - 4 * a ** 2) / (8 * np.pi * (nu - 1))
    f_galerkin[5, :] = 0
    f_galerkin[6, :] = 0
    return f_galerkin
...
def calc_displacements_and_stresses(
    ...
    if shape_function == "constant":
        ...
        f = quadratic_kernel_and_derivatives(x, y, a, nu)
